...
if [ -z "$changed_files" ]; then
  echo "[quality-council] no changed files"
  exit 0
fi
...
if [ -z "$package_names" ]; then
  echo "[quality-council] no package changes, build skipped"
  exit 0
fi

echo "[quality-council] build changed packages"
for pkg in $package_names; do
  ...
  lerna run build --scope "$scope" --include-dependencies
done
